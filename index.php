...
if (isset($_POST['toggle_LED'])) {
	$sql = "SELECT * FROM estado_rele;";
	$result   = mysqli_query($conn, $sql);
	$row  = mysqli_fetch_assoc($result);
	
	if($row['status'] == 0){
		$update = mysqli_query($conn, "UPDATE estado_rele SET status = 1 WHERE id = 1;");		
	}		
	else{
		$update = mysqli_query($conn, "UPDATE estado_rele SET status = 0 WHERE id = 1;");		
	}
}
